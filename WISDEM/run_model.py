...
def run_15mw(fname_wt_input):
    ...
    # Uses projected blade length, not arc length.  For now, no coning
    dtu_rotorD = 2*prob["blade.high_level_blade_props.blade_ref_axis"][-1,-1] + prob["hub.diameter"] #*np.cos(prob["hub.cone"])
    ...
    if not float_flag:
        ...
        overview['Floater hull mass [t]'] = 1e-3*prob['floatingse.platform_hull_mass']
        overview['Floater fixed ballast mass [t]'] = 1e-3*prob['floatingse.platform_ballast_mass'][0]
        overview['Floater fluid ballast mass [t]'] = 1e-3*prob['floatingse.variable_ballast_mass'][0]
        overview['Floater displacement [m^3]'] = prob['floatingse.platform_displacement']
        overview['Floater freeboard [m]'] = 15.0
